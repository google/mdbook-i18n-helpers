...
/// );
///
/// let groups = group_events(&events).unwrap();
/// assert_eq!(
///     groups,
///     vec![
///         Group::Skip(vec![
...
/// );
/// ```
pub fn group_events<'a>(events: &'a [(usize, Event<'a>)]) -> Result<Vec<Group<'a>>, CmarkError> {
    #[derive(Debug)]
    enum State {
        Translate(usize),
        Skip(usize),
...
pub fn reconstruct_markdown<'a>(
...
) -> Result<(String, State<'a>), CmarkError> {
    ...
    Ok((String::from(markdown.trim_start_matches('\n')), new_state))
}
...
/// );
/// ```
pub fn extract_messages(document: &str) -> Result<Vec<(usize, ExtractedMessage)>, CmarkError> {
...
mod tests {
    ...
    #[test]
    fn extract_events_code_block_empty() {
        let actual_events = extract_events(
            "\
            ```c++\n\
            ```",
            None,
    ...
    #[test]
    fn extract_events_code_block_comment() {
        let actual_events = extract_events(
            "\
            ```c++\n\
            // Fun fact\n\
    ...
    #[test]
    fn extract_events_code_block_comments() {
        let actual_events = extract_events(
            "\
            ```c++\n\
            // Fun fact\n\
    ...
    #[test]
    fn extract_events_code_block_comments_mixed() {
        let actual_events = extract_events(
            "\
            Text before.\n\
            \n\
        ...
        let expected_events = vec![
            ...
            (15, End(TagEnd::Paragraph)),
        ];
        assert_eq!(expected_events, actual_events);
    }

    #[test]
    fn extract_events_code_block_admonished_custom() {
        let actual_events = extract_events(
            "\
